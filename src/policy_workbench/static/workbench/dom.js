export const dom = {
  workbenchGrid: document.getElementById("workbench-grid"),
  workspacePanel: document.getElementById("workspace-panel"),
  workspacePanelContent: document.getElementById("workspace-panel-content"),
  btnToggleWorkspacePanel: document.getElementById("btn-toggle-workspace-panel"),
  mainTabEditor: document.getElementById("main-tab-editor"),
  mainTabActivation: document.getElementById("main-tab-activation"),
  mainPanelEditor: document.getElementById("main-panel-editor"),
  mainPanelActivation: document.getElementById("main-panel-activation"),
  runtimeModeBadge: document.getElementById("runtime-mode-badge"),
  runtimeAuthBadge: document.getElementById("runtime-auth-badge"),
  runtimeModeSelect: document.getElementById("runtime-mode-select"),
  runtimeModeUrl: document.getElementById("runtime-mode-url"),
  runtimeModeApply: document.getElementById("runtime-mode-apply"),
  runtimeLoginUsername: document.getElementById("runtime-login-username"),
  runtimeLoginPassword: document.getElementById("runtime-login-password"),
  runtimeLoginApply: document.getElementById("runtime-login-apply"),
  inventorySourceBadge: document.getElementById("inventory-source-badge"),
  editorSourceBadge: document.getElementById("editor-source-badge"),
  activationSourceBadge: document.getElementById("activation-source-badge"),
  editorPath: document.getElementById("editor-path"),
  currentPolicyId: document.getElementById("current-policy-id"),
  currentPolicyType: document.getElementById("current-policy-type"),
  currentPolicyNamespace: document.getElementById("current-policy-namespace"),
  currentPolicyKey: document.getElementById("current-policy-key"),
  currentPolicyVariant: document.getElementById("current-policy-variant"),
  currentPolicySchemaVersion: document.getElementById("current-policy-schema-version"),
  currentPolicyStatus: document.getElementById("current-policy-status"),
  currentPolicyVersion: document.getElementById("current-policy-version"),
  currentPolicyContentHash: document.getElementById("current-policy-content-hash"),
  currentPolicyUpdatedAt: document.getElementById("current-policy-updated-at"),
  currentPolicyUpdatedBy: document.getElementById("current-policy-updated-by"),
  currentPolicyActivation: document.getElementById("current-policy-activation"),
  fileEditor: document.getElementById("file-editor"),
  editorLintStatus: document.getElementById("editor-lint-status"),
  editorValidationDetails: document.getElementById("editor-validation-details"),
  inventoryWorld: document.getElementById("inventory-world"),
  inventoryPolicyType: document.getElementById("inventory-policy-type"),
  inventoryNamespace: document.getElementById("inventory-namespace"),
  inventoryStatus: document.getElementById("inventory-status"),
  inventoryCount: document.getElementById("inventory-count"),
  inventoryList: document.getElementById("inventory-list"),
  saveScopeMode: document.getElementById("save-scope-mode"),
  saveRolloutAllWorldsRow: document.getElementById("save-rollout-all-worlds-row"),
  saveRolloutAllWorlds: document.getElementById("save-rollout-all-worlds"),
  activationEnableRow: document.getElementById("activation-enable-row"),
  activationEnable: document.getElementById("activation-enable"),
  activationWorldDisplay: document.getElementById("activation-world-display"),
  activationClientProfile: document.getElementById("activation-client-profile"),
  activationScopeLabel: document.getElementById("activation-scope-label"),
  activationFilterPolicyType: document.getElementById("activation-filter-policy-type"),
  activationFilterNamespace: document.getElementById("activation-filter-namespace"),
  activationFilterStatus: document.getElementById("activation-filter-status"),
  activationFilterSearch: document.getElementById("activation-filter-search"),
  activationFilterCount: document.getElementById("activation-filter-count"),
  activationSelectedMapping: document.getElementById("activation-selected-mapping"),
  activationSetStatus: document.getElementById("activation-set-status"),
  btnActivationApplyStatus: document.getElementById("btn-activation-apply-status"),
  activationSaveSummary: document.getElementById("activation-save-summary"),
  activationList: document.getElementById("activation-list"),
  themeToggle: document.getElementById("theme-toggle"),
  statusText: document.getElementById("status-text"),
  statusSource: document.getElementById("status-source"),
  btnRefreshInventory: document.getElementById("btn-refresh-inventory"),
  btnEditFile: document.getElementById("btn-edit-file"),
  btnCloseFile: document.getElementById("btn-close-file"),
  btnValidateFile: document.getElementById("btn-validate-file"),
  btnSaveFile: document.getElementById("btn-save-file"),
  btnReloadFile: document.getElementById("btn-reload-file"),
  btnOpenActivationTab: document.getElementById("btn-open-activation-tab"),
  btnRefreshActivation: document.getElementById("btn-refresh-activation"),
};
